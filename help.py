...
async def help(message, data, command_args, mention_ids):
    msg = ''
    ...
    try : 
        ...
        if attack_index[0] == common.ATTACK_MAIN:
            ...
            if attack_index[1] == common.NO_ATTACK_NUMBER:
                ...
                for i in range(0, common.ATTACK_MAX + 1):
                    ...
                    if i == common.ATTACK_MAX or res_len < i:
                        raise common.CommandError(messages.error_help_main_full)
                    # 未凸の凸情報を取得
                    if atk[i][common.DAILY_MEMBER_ATTACK_STATUS_KEY] == common.DAILY_ATTACK_STATUS_NONE:
                        if res[i][0][common.RESERVATION_STATUS_KEY] != common.RESERVE_STATUS_HELP :
            ...
            if current_lap_key in dic:
                ...
                for i in range(0, len(rs)):
                    ...
                    if rs[i][common.RESERVATION_ID_KEY] == target_id and rs[i][common.RESERVATION_BRANCH_KEY] == 0 and rs[i][common.RESERVATION_STATUS_KEY] != common.RESERVE_STATUS_HELP:
                        target_res = rs[i]
                        flg = False
                        break
            ...
            # 本凸予約が無い場合エラー
            if flg:
               raise common.CommandError(messages.error_help_impossible)
        ...
        # 持ち越しの場合  
            ...
            if attack_index[1] == common.NO_ATTACK_NUMBER:
                ...
                for i in range(0, common.ATTACK_MAX + 1):
                    if i == common.ATTACK_MAX or res_len < i:
                        raise common.CommandError(messages.error_help_carry_full)

                    if atk[i][common.DAILY_MEMBER_ATTACK_STATUS_KEY] == common.DAILY_ATTACK_STATUS_CARRY_OVER :
                        if res[i][1][common.RESERVATION_STATUS_KEY] != common.RESERVE_STATUS_HELP :
                            target_attack_index = i
                            break
            ...
            if current_lap_key in dic:
                ...
                for i in range(0, len(rs)):
                    # 予約があった場合は予約情報を保存
                    if rs[i][common.RESERVATION_ID_KEY] == target_id and rs[i][common.RESERVATION_BRANCH_KEY] == 1 and rs[i][common.RESERVATION_STATUS_KEY] != common.RESERVE_STATUS_HELP:
                        target_res = rs[i]
                        flg = False
                        break
        ...
        if (res[org_attack_index][org_branch_index][common.RESERVATION_STATUS_KEY] == common.RESERVE_STATUS_HELP):
            raise common.CommandError(messages.error_help_already)
        
        # 予約情報のstatus変更する
        res[org_attack_index][org_branch_index][common.RESERVATION_STATUS_KEY] = common.RESERVE_STATUS_HELP
