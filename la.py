...
async def la(message, data, command_args, mention_ids):
    ...
    try: 
        ...
        if current_lap_key in dic:
            ...
            for r in rs:
                # 予約状態のものがあった場合は、周を移動する 20210909 startとhelp statusの条件追加
                if (r[common.RESERVATION_STATUS_KEY] == common.RESERVE_STATUS_RESERVED or r[common.RESERVATION_STATUS_KEY] == common.RESERVE_STATUS_START or r[common.RESERVATION_STATUS_KEY] == common.RESERVE_STATUS_HELP):
                    ...
                    #凸状況が開始 or 救援の場合　→　予約へ変更
                    data[common.DATA_DAILY_KEY][common.DAILY_MEMBER_KEY][member_key][common.DAILY_MEMBER_RESERVATION_KEY][r[common.RESERVATION_SEQ_KEY]][r[common.RESERVATION_BRANCH_KEY]][common.RESERVATION_STATUS_KEY] = common.RESERVE_STATUS_RESERVED
