        ...
        for s in data[common.DATA_MEMBER_KEY]:
            ...
            for m in mention_ids:
                if m == s[common.MEMBER_ID_KEY]:
                    flg = False
                    # 20210912 ロール削除処理追加
                    await common.id_remove_role(message.guild,m)
...
async def allremove(message, data, command_args, mention_ids):
    try:
        # メッセージを送った本人が管理者かどうか
        if not message.author.id in [common.ADMINISTRATOR_ID, common.ADMINISTRATOR_ID1] :
            raise common.CommandError(messages.error_not_admin)
        ...
        if num == 0:
            # ラピュタメッセージ送信
            wait3 = discord.File(common.MUSUKA_IMAGE_WAIT3)
            eyes = discord.File(common.MUSUKA_IMAGE_EYES)

            # ３分間まってやる
            await message.channel.send(messages.wait_3min)     
            await message.channel.send(file=wait3)
            await common.sleep_late(3)

            # 目が～目が～
            await message.channel.send(messages.my_eyes_cant_see)
            await message.channel.send(file=eyes)
            await common.sleep_late(2)
        
        else:
            for t in messages.Castle_in_the_Sky:
                s = f'**```{t}```**\n'                
                await message.channel.send(s)
                await common.sleep_late(2.5)
        
        # ロール全消し
        await common.add_remove_role(data, message.guild, True)

        # dailyファイル削除
        common.delete_file(common.DATA_DAILY_PATH)

        # memberファイル削除
        common.delete_file(common.DATA_MEMBER_PATH)

        # bossファイル削除
        common.delete_file(common.DATA_BOSS_PATH)
